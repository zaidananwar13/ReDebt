...
//                    createScene()
    ...
    func normalizeSize(horizontal: Double, min: Double, max: Double) -> Double
    {
        print(min)
        return map(minRange: abs(min), maxRange: abs(max), minDomain: 50, maxDomain: 150, value: abs(horizontal))
    }
    ...
    func updateBubble() {
        ...
        for person in persons {
            if person.totalDebt != 0 {
                ...
                let size = normalizeSize(horizontal: person.totalDebt, min: minx, max: maxx)
                let _ = print("[updateBubbble][size] \(normalizeSize(horizontal: person.totalDebt, min: minx, max: maxx))")
