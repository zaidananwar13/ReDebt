...
open class SIFloatingCollectionScene: SKScene {
    ...
    override open func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if let touch = touches.first as UITouch? {
            ...
            let beta = sqrt(pow(lin.x, 2) + pow(lin.y, 2))
            distanceX = beta == 0 ? 0 : (distanceX / beta)
            distanceY = beta == 0 ? 0 : (distanceY / beta)
            ...
            for node in floatingNodes {
                let weightNode = node.frame.size.width / 2
                let heightNode = node.frame.size.height / 2
                var direction = CGVector(
                    dx: pushStrength * distanceX,
                    dy: pushStrength * distanceY
                )
                if restrictedToBounds {
                    if !(-weightNode...(size.width + weightNode) ~= node.position.x) && (node.position.x * distanceX) > 0 {
                        direction.dx = 0
                    }
                    if !(-heightNode...(size.height + heightNode) ~= node.position.y) && (node.position.y * distanceY) > 0 {
                        direction.dy = 0
                    }
                }
