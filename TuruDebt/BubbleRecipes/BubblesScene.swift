...
class BubblesScene: SIFloatingCollectionScene {
    ...
    override func addChild(_ node: SKNode) {
        if node is BubbleNode {
            var horizontal = CGFloat.random(min: -bottomOffset, max: -node.frame.size.width)
            let vertical = CGFloat.random(
                min: 50,
                max: 150
            )

            if floatingNodes.count % 2 == 0 || floatingNodes.isEmpty {
                horizontal = CGFloat.random(
            ...
            node.position = CGPoint(x: horizontal, y: vertical)
        }
        super.addChild(node)
    }
